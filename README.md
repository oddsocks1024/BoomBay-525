...
## 3D Printable Case
The STL files for the case are included in this repo. The PCB is designed to be mounted into this case which can than be inserted into the 5.25" drive bay of a computer. Please
also consult the images in addition to these instructions.

 * Print the case in it's normal orientation.
 * Print supports are only needed for the volume, LED and headphone cut-outs on the front. Do **not** add support for the speaker grille or side mounting screw holes.
 * Print 4 x Screw Nuts which are used for holding the PCB in place.
 * Once printed, cleanly remove the printing supports from the volume, LED and headphone cut-outs on the front of the case.
 ...
 * Turn the screw so that it starts to tightens, biting into the nut and holding the PCB in place. Continue until it's flush.
 * Repeat for the remaining 3 screws.
 
## Mounting in a Computer
 * Find an empty 5.25" drive bay.
 * Insert the BoomBay 525 into the drive bay.
 * Connect the Power.
 * Connect the 3.5mm audio jack cable to the audio-in on the BoomBay 525.
 ...
 * Screw the case into place in the case.
